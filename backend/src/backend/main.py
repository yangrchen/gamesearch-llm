...
class QueryEvaluationOutput(BaseModel):
    ...
    violation_reason: str | None = Field(
        description="Violation reason for why the query was not allowed", default=None
    )
...
class QueryState(BaseModel):
    ...
    query_type: UserQueryType = UserQueryType.Hard
...
def compile_agent_workflow():
    ...
    workflow.add_edge("handle_allowed", END)
    workflow.add_conditional_edges(
        "handle_allowed",
        route_allowed_by_type,
        {UserQueryType.Hard: "handle_hard_query", UserQueryType.Vector: END},
    )
...
async def search(user_query: QueryState):
    ...
    logger.info(f"Initial query: {user_query}")
